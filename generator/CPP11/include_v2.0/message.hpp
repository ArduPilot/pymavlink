...
/*
  WIP is different from deprecated/superseded above: if MAVLINK_MSG_TYPE_WIP
  is defined as unavailable() (the closest strict option, since error() is
  function-only), then unlike deprecated(), neither GCC nor clang lets a diagnostic pragma silence
