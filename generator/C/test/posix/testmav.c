...
static void test_sysid32(void)
{
    ...
    for (uint8_t i=0; i<2; i++) {
        for (uint8_t j=0; j<2; j++) {
            ...
            assert(((msg.incompat_flags & MAVLINK_IFLAG_TARGET32) != 0) == targetted);
            assert(msg.compat_flags == 0);
            assert(msg.sysid == sysid);

            // getters must return the full target, payload byte must be
            // 255 when the target is in the extended header
            mavlink_command_long_t pkt;
            mavlink_msg_command_long_decode(&msg, &pkt);
            uint32_t effective_target;
            assert(mavlink_msg_get_target_system(&msg, &pkt.target_system, &effective_target));
            ...
            assert((uint8_t)_MAV_PAYLOAD(&msg)[entry->target_system_ofs] == (targetted?MAVLINK_TARGET_SYSTEM_SENTINEL:target));

            // The target sentinel cannot be mistaken for the broadcast value zero;
            // use the getter above to obtain the full extended target.
            assert(pkt.target_system == (targetted?MAVLINK_TARGET_SYSTEM_SENTINEL:target));
            ...
            const uint16_t header_len = MAVLINK_NUM_HEADER_BYTES +
                ...
                (targetted?MAVLINK_TARGET32_HEADER_EXTRA:0);
            ...
            mavlink_msg_command_long_decode(&rmsg, &pkt);
            assert(mavlink_msg_get_target_system(&rmsg, &pkt.target_system, &effective_target));
            assert(effective_target == target);
    ...
    // A frame with small IDs must be byte-identical to a MAVLink2.0 frame.
    {
        mavlink_message_t msg;
        memset(&msg, 0, sizeof(msg));
    ...
    // TARGET32 is valid without SYSID32. In that form the target system
    // always occupies four bytes, followed by target component.
    {
        mavlink_message_t msg;
        memset(&msg, 0, sizeof(msg));
        mavlink_msg_command_long_pack(42, 11, &msg, 7, 250, 300, 1,
        ...
        msg.target_sysid = 7;
        _MAV_PAYLOAD_NON_CONST(&msg)[entry->target_system_ofs] = 0;
        _MAV_PAYLOAD_NON_CONST(&msg)[entry->target_component_ofs] = 250;

        uint8_t header[MAVLINK_MAX_HEADER_LEN];
        const uint8_t header_len = mavlink_header_to_send_buffer(header, &msg);
        assert(header_len == MAVLINK_NUM_HEADER_BYTES + MAVLINK_TARGET32_HEADER_EXTRA);
        assert(header[MAVLINK_NUM_HEADER_BYTES] == 7);
        assert(header[MAVLINK_NUM_HEADER_BYTES + 1] == 0);
        assert(header[MAVLINK_NUM_HEADER_BYTES + 2] == 0);
        assert(header[MAVLINK_NUM_HEADER_BYTES + 3] == 0);

        uint16_t crc = crc_calculate(&header[1], header_len - 1);
        ...
        assert(parse_buffer(buf, len, &rmsg, NULL, NULL) == MAVLINK_FRAMING_OK);
        assert(rmsg.sysid == 42);
        assert(rmsg.target_sysid == 7);
        assert(mavlink_msg_get_target_sysid(&rmsg, entry) == 7);
        assert(mavlink_msg_get_target_compid(&rmsg, entry) == 250);
        assert(mavlink_msg_command_long_get_target_component(&rmsg) == 250);
        mavlink_command_long_t pkt;
        mavlink_msg_command_long_decode(&rmsg, &pkt);
        assert(pkt.target_system == 7);
        assert(pkt.target_component == 250);

        uint8_t forwarded[MAVLINK_MAX_PACKET_LEN];
        assert(mavlink_msg_to_send_buffer(forwarded, &rmsg) == len);
        assert(memcmp(buf, forwarded, len) == 0);
    }
...
static void test_sysid32_send(void)
{
    ...
    assert((rmsg.incompat_flags & MAVLINK_IFLAG_TARGET32) != 0);
...
static void test_sysid32_signing(void)
{
    ...
    mavlink_msg_command_long_pack_status(0x0A000001UL, 11, &status, &msg, 0x0A000002UL, 250, 300, 1,
                                         1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f);
    assert(msg.incompat_flags == (MAVLINK_IFLAG_SIGNED|MAVLINK_IFLAG_SYSID32|MAVLINK_IFLAG_TARGET32));
    const uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);
