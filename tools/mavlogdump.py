    ...
    while True:
        ...
        if output is not None:
            ...
            if (isbin or islog) and m_type in ["FMTU", "MULT", "UNIT"]:
                output.write(m.get_msgbuf())
                continue
            if (isbin or islog) and (m_type == "PARM" and parms):
                output.write(m.get_msgbuf())
                continue
            if m_type == 'PARAM_VALUE' and parms:
        ...
        if format == 'json':
            ...
            # do nothing
            pass
        elif verbose and istlog:
            mavutil.dump_message_verbose(sys.stdout, m)
            print("")
        elif verbose and hasattr(m,"dump_verbose"):
