...
of the protocol, with optional remapping of names and values.
...
parser.add_argument("-m", "--mapping", help="JSON dictionary with mapping of new values from old, or path to JSON file", default="{}")
...
args_mapping = args.mapping  # type: str
...
if args_from is None and args_to is None:
    parser.error("Specify at least 'from' or 'to' definition, with the unspecified one assumed to be installed pymavlink")
...
def protocol_module(identifier, repository, dialect):
    ...
    if identifier is None:
        ...
        mavgen.mavgen(mavgen.Opts(os.path.join(out_path, "mavlink_generated"), language="Python3", validate=False, wire_protocol="1.0" if args_mav10 else "2.0"), [dialect_path])

        sys.path.insert(1, out_path)
        return importlib.import_module("mavlink_generated")
...
while True:
    ...
    mtype = message_from.get_type()
    if mtype not in seen_messages:
        print(mtype)
        seen_messages[mtype] = 0
    seen_messages[mtype] += 1
